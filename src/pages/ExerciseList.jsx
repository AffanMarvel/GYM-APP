...
export default function ExerciseList() {
  ...
  const currentExercises = exercises[muscle] || [];
    ...
    <div className="min-h-screen pb-32" style={{ background: '#06060d' }}>
      <div className="p-5 slide-up max-w-lg mx-auto space-y-6">
        <header className="flex items-center space-x-4 pt-2">
          <Link to="/workout" className="p-2.5 bg-gym-card rounded-xl border border-white/5 shadow-lg active:scale-95 transition-all">
            <ChevronLeft size={22} className="text-white" />
          </Link>
          <div>
            <h1 className="text-3xl font-black text-white capitalize">{muscle}</h1>
            <p className="text-[10px] font-bold text-gym-neon uppercase tracking-[0.2em]">{currentExercises.length} Exercises Available</p>
          </div>
        </header>

        <div className="relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gym-muted" size={18} />
          <input type="text" placeholder={`Search ${muscle} exercises...`} value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-[#141425] border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-white text-sm focus:outline-none focus:border-gym-neon/50 font-bold shadow-lg transition-all" />
        </div>

        <div className="grid grid-cols-1 gap-6 mt-6">
          {filtered.length === 0 ? (
            <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-3xl opacity-50">
              <p className="text-gym-muted font-bold tracking-widest uppercase text-xs">No exercises found</p>
            </div>
          ) : (
            filtered.map((ex, i) => (
              <div key={i} className="group bg-[#141425] rounded-3xl overflow-hidden border border-white/5 hover:border-gym-neon/30 transition-all duration-300 shadow-[0_8px_30px_rgb(0,0,0,0.4)]">
